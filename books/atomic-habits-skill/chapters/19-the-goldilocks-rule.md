...
First stabilize attendance so that difficulty can be adjusted against a real practice record. Then identify the current edge: what can the user do reliably, and what slightly harder version would demand attention without creating repeated defeat? Change one variable at a time—complexity, speed, duration, feedback, or context—so the effect is interpretable. Keep the routine and cue recognizable while refreshing the challenge. If motivation drops, test for boredom, excessive difficulty, or a reward that has become too distant before redesigning the entire system.

1. Choose a difficulty that permits frequent wins.
2. Increase challenge in small increments.
3. Keep a minimum version for bad days.
4. Add deliberate variation or a new target when boredom appears.

## Pitfall

Changing the entire routine whenever excitement dips sacrifices the repetition that makes the habit useful. Preserving the system does not mean tolerating a challenge that is chronically too easy or too hard. Another mistake is raising difficulty so quickly that the user receives mostly failure signals and stops returning. Adjust the edge of difficulty while keeping the cue and minimum version stable. The goal is durable engagement, not permanent novelty.
