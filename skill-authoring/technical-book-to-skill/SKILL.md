---
name: technical-book-to-skill
description: "Use when a technical book, academic paper, API manual, programming guide, or other code-, table-, formula-, or diagram-heavy source should become a reusable agent skill with source-grounded chapter notes and optional Anki cards."
---
...
# Technical Book-to-Skill Converter

Transform technical written knowledge into actionable agent skills by extracting structure — not producing thin summaries.
...
> "technical-book-to-skill requires a supported document path, folder, or glob pattern. Usage: `technical-book-to-skill <path-to-document-folder-or-glob>... [skill-name-slug]`"
...
This variant defaults to technical mode because its intended inputs are technical books, academic papers, API manuals, programming guides, and other sources with code, tables, formulas, or diagrams. Store `BOOK_TYPE=technical` unless the user explicitly says the source is mostly prose, in which case use `BOOK_TYPE=text` and retain the general prose template.
...
SCRIPT_PATH=""
for candidate in \
  "$HOME/.copilot/skills/technical-book-to-skill/scripts/extract.py" \
  "$HOME/.agents/skills/technical-book-to-skill/scripts/extract.py" \
  "$HOME/.claude/skills/technical-book-to-skill/scripts/extract.py" \
  ".github/skills/technical-book-to-skill/scripts/extract.py" \
  ".claude/skills/technical-book-to-skill/scripts/extract.py" \
  ".agents/skills/technical-book-to-skill/scripts/extract.py" \
  "$HOME/.config/agents/skills/technical-book-to-skill/scripts/extract.py" \
  "$HOME/.config/amp/skills/technical-book-to-skill/scripts/extract.py" \
...
if [ -z "$SCRIPT_PATH" ]; then
  echo "Could not find scripts/extract.py for technical-book-to-skill" >&2
  exit 1
fi
