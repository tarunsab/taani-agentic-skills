...
<h1 align="center">technical-book-to-skill</h1>
...
> This is the `technical-book-to-skill` variant of the converter. It keeps the same extraction, on-demand, source-coverage, and Anki structure while defaulting to technical extraction for code, tables, formulas, and diagrams. It does not pad short source treatments or invent detail.
...
1. **Point** it at a file, folder, or glob — `/technical-book-to-skill ./my-book.pdf`
...
**technical-book-to-skill solves this by turning the technical source into a structured skill your agent loads on demand.**
...
Running `/technical-book-to-skill your-book.pdf` (or a folder, glob, or list of files) creates a full skill in your agent's skills directory (`~/.copilot/skills/<slug>/` for Copilot CLI, `~/.agents/skills/<slug>/` for Amp or cross-agent, `~/.claude/skills/<slug>/` for Claude Code):
...
`/technical-book-to-skill <path|folder|glob> [skill-name]` — plus analyze-only, generate-from-analysis, update/fold-in, and optional Anki export modes. After a conversion, the converter can publish the skill to GitHub (private by default) so any host installs it with `npx skills add`.
